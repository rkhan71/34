...
for row in allrows: #going through every single row in the table lists and making it the first edge of our square
    ...
    for row2 in ln2: #going through every potential second edge
        cursor.execute(f'SELECT * FROM lists WHERE fst = {row2[3]};') #finding potential third edges
        ln3 = cursor.fetchall()
        ...
        if len(ln3) > 0 and row2[1] not in usednums and row2[2] not in usednums and row2[3] not in usednums: 
            usednums += [row2[1], row2[2], row2[3]]
            for row3 in ln3: 
                cursor.execute(f'SELECT * FROM lists WHERE fst = {row3[3]} AND fth = {row[0]};')
                ln4 = cursor.fetchall()
                if len(ln4) > 0 and row3[1] not in usednums and row3[2] not in usednums and row3[3] not in usednums:
                    usednums += [row3[1], row3[2], row3[3]]
                    for row4 in ln4:
